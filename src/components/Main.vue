<template>
  <div class=" container jumbotron">
    ...
    <div v-if="isFinish"  class="alert alert-primary">
      ...
      <button @click="newGame" class="mt-5 btn btn-success btn-lg btn-block">YENİ OYUN  </button>
    </div>
    <div v-else>
      <div class="card">
        <div class="card-body">
          <span v-for="(word,key) in words.filter((data,index)=>index<20)" :key="key" :class="key!=0 || writingWordControl" class="words ml-2">
              {{word}}
          </span>
        </div>
      </div>
